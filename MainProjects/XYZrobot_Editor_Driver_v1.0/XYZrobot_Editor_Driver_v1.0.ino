...
_header_check:
  while((_temp = Serial.read()) == -1){
    if(seq_trigger){
      ...
      for(_i = 0; _i < BOLIDE_Robot.poseSize; _i++) BOLIDE_Robot.setNextPose(_i+1, poses[pose_index][_i]);
      BOLIDE_Robot.interpolateSetup(sequence[SeqPos].time);
      while(BOLIDE_Robot.interpolating) BOLIDE_Robot.interpolateStep();
      SeqPos++;
  ...
  goto _header_check;
_packet_start:
  pBuffer[header_address] = _temp; pIndex = 1; _enable_timer4(); _reset_timer4(); packet_timeout_status = false;
  while((_temp = Serial.read()) == -1){
    if(packet_timeout_status) {packet_error_feedback(0x00); goto _header_check;}
  }
  pBuffer[length_address] = _temp; pLength = _temp; pIndex++; _reset_timer4(); packet_timeout_status = false;
  while((_temp = Serial.read()) == -1){
    if(packet_timeout_status) {packet_error_feedback(0x00); goto _header_check;}
  }
  pBuffer[CMD_address] = _temp; pCMD = _temp; pIndex++; _reset_timer4(); packet_timeout_status = false;
  while(1){
    while((_temp = Serial.read()) == -1){
      if(packet_timeout_status) {packet_error_feedback(0x00); goto _header_check;}
    }
    pBuffer[pIndex] = _temp; pIndex++; _reset_timer4(); packet_timeout_status = false;
    if(pIndex == pLength) {clear_buffer(); _disable_timer4(); _reset_timer4(); goto _packet_finish;}
  }
_packet_finish:
  if(pBuffer[pIndex-1] == packet_tail){     // check motor tail
    if(pCMD == CMD_init_motor){
      ...
      if(SeqProcessCnt == SEQ_Process_load_SEQCnt){
        ...
        if(SeqCnt == seq_pSeqCnt){packet_error_feedback(0x02); SeqCnt = 0; seq_trigger = true; SeqProcessCnt = SEQ_Process_load_SEQ;}
        else packet_error_feedback(0x01);
      }
      else packet_error_feedback(0x00);
    }
