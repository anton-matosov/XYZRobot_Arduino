...
_receive_Data:
  ...
  Serial.print("ID_setting : ");Serial.println(ID_Setting);
  while(Serial.read() != -1);
  status_check_1 = 0x00; status_check_2 = 0x00;
  temp = 0x00;
  ...
  WriteDataRAM1(A1_16_Broadcast_ID, RAM_LED_Control, LED_NO_MASK); delay(20);
  WriteDataRAM1(A1_16_Broadcast_ID, RAM_Alarm_LED_Policy, 0xFF); delay(20);
  WriteDataRAM1(A1_16_Broadcast_ID, RAM_LED_Control, LED_RED_MASK); delay(20);
  while(temp != ID_Setting){
    SetID(A1_16_Broadcast_ID, ID_Setting); delay(20);
    temp = ReadDataEEP1(ID_Setting, EEP_sID);
    if((status_check_2 & timeout_mask)) goto _Timeout_Feedback;
  }
  status_check_1 |= ID_Setting_mask;
  while(abs(temp-724) > error_range){
    SetPositionI_JOG(ID_Setting, 0x00, 724); delay(500);
    temp = ReadPosition(ID_Setting);
    if((status_check_2 & timeout_mask)) goto _Timeout_Feedback;
  }
  while(abs(temp-512) > error_range){
    SetPositionI_JOG(ID_Setting, 0x00, 512); delay(500);
    temp = ReadPosition(ID_Setting);
    if((status_check_2 & timeout_mask)) goto _Timeout_Feedback;
  }
  while(abs(temp-300) > error_range){
    SetPositionI_JOG(ID_Setting, 0x00, 300); delay(500);
    temp = ReadPosition(ID_Setting);
    if((status_check_2 & timeout_mask)) goto _Timeout_Feedback;
  }
  while(abs(temp-512) > error_range){
  ...
  WriteDataRAM1(ID_Setting, RAM_LED_Control, LED_GRN_MASK); delay(20);
_finish_test:
  _disable_timer4();
  _reset_timer_counter();
